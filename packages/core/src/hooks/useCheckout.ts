...
import type { CheckoutError, CreateCheckoutResult } from '../types'

export function useCheckout() {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<CheckoutError | null>(null)

  const createCheckout = useCallback(async (params: CreateCheckoutParams): Promise<CreateCheckoutResult> => {
    setIsLoading(true)
    setError(null)
